...
if arquivo:
    ...
    for alvo in volumes_alvo:
        candidatos = df[df["Volume"] >= 1].copy()
            ...
            for combinacao in itertools.combinations(candidatos.iterrows(), r):
                ...
                if abs(vol_total - alvo) <= 10:
                    ...
                    if lucro_por_saca >= lucro_minimo:
                        ...
                        # Subtrair volumes
                        for i, (idx, _) in enumerate(combinacao):
                            if idx in df.index:
                                df.at[idx, "Volume"] -= volumes[i]
                                if df.at[idx, "Volume"] <= 0:
                                    indices_a_remover.add(idx)

    # Remoção após o loop
    df.drop(index=indices_a_remover, inplace=True)
    ...
    estoque_residual = df[df["Volume"] > 0].copy()
